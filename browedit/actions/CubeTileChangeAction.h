...
{
	std::map<Gnd::Cube*, std::array<int, 3>> oldValues;
	std::map<Gnd::Cube*, std::array<int, 3>> newValues;
	std::vector<glm::ivec2> selection;
	bool shadowDirty;
public:
	CubeTileChangeAction(glm::ivec2 tile, Gnd::Cube*, int tileUp, int tileFront, int tileSide);
	CubeTileChangeAction(const std::vector<glm::ivec2>& selection, const std::map<Gnd::Cube*, std::array<int, 3>>& oldValues, const std::map<Gnd::Cube*, std::array<int, 3>>& newValues);
